...
# Notes: Re-ran by Mike A in May 2022 to group tags by strategy (DSR, NRR) rather
# than by life stage (Smolt, Parr, Presmolt)
  ...
  mutate(stan_summ = map(stan_fit,
                         .f = function(fit) {
                           fit_summ = try(summary(fit)$summary %>%
                                            ...
                                            add_column(strategy = NA_character_,
                                                       .after = "param") %>%
                                            mutate(strategy = if_else(grepl(",1\\]$", param),
                                                                      "DSR",
